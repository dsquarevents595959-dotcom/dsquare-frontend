...
const DjLightingVisual = () => {
  ...
  const fetchDjLightingServices = async () => {
    try {
      const response = await fetch('/api/service-cards/dj-lighting-visual');
      const result = await response.json();
      if (result.success) {
        setDjLightingServices(result.data);
      }
    } catch (error) {
      console.error('Failed to fetch DJ & Lighting & Visual services:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <section className="min-h-screen bg-slate-950 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center py-12">
            ...
            <p className="text-slate-400 mt-2">Loading DJ & Lighting & Visual services...</p>
          </div>
        </div>
      </section>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-300 max-w-3xl mx-auto">
            Professional DJ, lighting, and visual effects for your events
          </p>
        </div>
        ...
        {djLightingServices.length === 0 && !loading && (
          <div className="text-center py-12">
            <p className="text-slate-400 text-lg">No DJ & Lighting & Visual services available at the moment.</p>
          </div>
        )}
      </div>
    </section>
