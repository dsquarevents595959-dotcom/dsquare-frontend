...
import { FaBirthdayCake, FaGift, FaStar, FaUsers } from 'react-icons/fa';
...
const Birthdays = () => {
  ...
  useEffect(() => {
    fetchBirthdayServices();
  }, []);

  const fetchBirthdayServices = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/services/birthdays`);
      const result = await response.json();
      if (result.success) {
        setBirthdayServices(result.data);
      }
    } catch (error) {
      console.error('Failed to fetch birthday services:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <section className="min-h-screen bg-slate-950 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center py-12">
            ...
            <p className="text-slate-400 mt-2">Loading birthday services...</p>
          </div>
        </div>
      </section>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-300 max-w-3xl mx-auto">
            Complete birthday celebration services for all ages
          </p>
        </div>
        ...
        {birthdayServices.length === 0 && !loading && (
          <div className="text-center py-12">
            <p className="text-slate-400 text-lg">No birthday services available at the moment.</p>
          </div>
        )}
      </div>
    </section>
